...
// import metrosList from '../public/metros_list.json'
...
export default function MetroPlots ({ metroPath, metrosList, setTitle }) {
  console.log(metroPath)
  console.log(metrosList)
  // console.log(data)

  const router = useRouter()

  const { data: metrosListResponse } = useSWR('/metros_list.json')
  // const metrosListResponse = metrosList
  ...
  useEffect(() => {
    if (optionVal !== null) {
      setTitle(optionVal.metro_name)
    }
  }, [optionVal])

  const url = getJsonUrl(metroPath)
  // const { data } = useSWR(url, () => window.fetch(url).then(res => res.json()), { initialData: initialData })
