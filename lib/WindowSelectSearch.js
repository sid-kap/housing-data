...
const WindowSelectSearch = forwardRef(({
  ...
  const flattenedOptions = makeIndexMap(options)
  ...
  const renderItem = function (row) {
    ...
    if (isGroup) {
      return (
        <div key={option.value} className={cls('group-header')} style={style}>{renderGroupHeader(option.name)}</div>
      )
    } else {
      return (
        <Option
